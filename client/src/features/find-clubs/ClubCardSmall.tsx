...
const ClubCardSmall: React.FC<ClubCardProps> = ({ name, tagline, id, isSelected, tags = [], onClick }) => {
    ...
    return (
        ...
        >
            <div className="flex items-center gap-3 mb-2">
                <div className={`w-14 h-14 rounded-full flex items-center justify-center text-sm font-semibold ${getClubColors(id)}`}>
                    {clubInitials}
                </div>
                <div className="flex-1 overflow-hidden">
                    ...
                    <div className="mt-2 flex gap-2 overflow-hidden whitespace-nowrap text-ellipsis">
                        {tags.map(tag => (
                            <span key={tag._id} className="bg-primary-container text-primary rounded-full px-3 py-1 text-xs font-semibold">
                                {tag.text}
                            </span>
                        ))}
                    </div>

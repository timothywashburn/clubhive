                ...
                <div className="bg-surface rounded-lg shadow">
                    ...
                    <div className="divide-y divide-outline-variant">
                        ...
                        {[
                            {
                                type: 'event',
                                message: 'New event: Club Meeting tomorrow at 7 PM',
                                time: '2 hours ago',
                                unread: true,
                            },
                            {
                                type: 'club',
                                message: 'You were accepted into Photography Club',
                                time: '1 day ago',
                                unread: true,
                            },
                            {
                                type: 'reminder',
                                message: 'Reminder: Soccer practice today at 5 PM',
                                time: '2 days ago',
                                unread: false,
                            },
                            {
                            ...
                            <div key={i} className={`px-6 py-4 ${notification.unread ? 'bg-primary/10' : ''}`}>
                                <div className="flex items-start">
                                    <div
                                        className={`w-2 h-2 rounded-full mt-2 mr-3 ${notification.unread ? 'bg-primary' : 'bg-outline-variant'}`}
                                    />
                                    <div className="flex-1">
                                        <p className="text-on-surface">{notification.message}</p>
                                        <p className="text-sm text-on-surface-variant mt-1">{notification.time}</p>
                                    </div>
                                </div>
                            </div>
                        ))}
